...
function img(flag) {
  return flag === "" ? "" : "text-box-h2-active";
}
...
function Textbox(props) {
  ...
  const item = items.map((item) => (
    <div className="text-box" key={item.id}>
      <h2 className={img(item.photo)} onClick={item.setPhoto}>
        {item.title}
      </h2>
      <p>{item.text}</p>
    </div>
...
class Post extends React.Component {
  constructor(props) {
    ...
    this.changeCurPhoto = this.changeCurPhoto.bind(this);
  }

  changeCurPhoto(photo) {
    this.setState(() => ({
      curPhoto: photo,
    }));
  }
    ...
    return (
      <div className="postext-container">
        ...
        <div className="content-container">
          <Textbox
            textItems={this.state.section}
          />
          <div className="post-photo-container">
            <img alt="img" src={this.state.curPhoto} />
          </div>
        </div>
      </div>
    );
